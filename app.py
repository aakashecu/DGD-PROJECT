...
def extract_pdf_text_and_image_info(uploaded_file, dpi=300):
    ...
    try:
        ...
        for idx, pil in enumerate(pil_pages, start=1):
            ...
            signature_present = (
                label_present and signature_ink_present and ink_conf >= 0.75
            )
...
def load_sop_text():
    sop_path = Path(__file__).parent / "data" / "DGD AND MSDS CHECKING SOP.docx"
    ...
    try:
        ...
        st.error(f"Failed to load SOP document: {e}")
        return ""
...
if run_validation:
    ...
    with st.spinner("Extracting & validating..."):
        ...
        try:
            ...
            llm_signature_status = parsed.get("signature_check", {}).get("status", "").lower()
...
# ----------------- Deterministic signature override (SAFE & FINAL) -----------------
try:
    ...
    # 🚨 RULE: NEVER upgrade if LLM says signature is missing
    if "missing" in llm_signature_status:
        parsed["signature_check"] = {
            "status": "Signature missing",
            "details": parsed["signature_check"].get(
                "details",
                "Declarant signature field is not signed."
            )
        }
...
with right:
    ...
    ERROR_WORDS = [
        ...
        "not found", "not signed", "inner packing missing"
    ]
...
with left:
    ...
    def icon(status, key=None, item=None):
        ...
        # ----------------------------------------
        # HARD OVERRIDE — SIGNATURE (DO NOT FALL THROUGH)
        # ----------------------------------------
        if key == "signature_check":
            if "missing" in s or "not signed" in d:
                return "🔴"
            if "present" in s:
                return "🟢"
            return "🔴"
        ...
        # ----------------------------------------
        # HARD OVERRIDE — MSDS VALIDITY (DO NOT FALL THROUGH)
        # ----------------------------------------
        if key == "msds_validity":

            # 1️⃣ Explicit INVALID from LLM → RED
            if "invalid" in s:
                return "🔴"

            # 2️⃣ Normalize dates to a list
            dates = item.get("msds_dates_found", [])
            if isinstance(dates, str):
                dates = [dates]

            # 3️⃣ Date-based check
            ...
            # 4️⃣ If explicitly valid
            if any(w in s for w in ["valid", "ok", "within"]):
                return "🟢"
